    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < A.Count; i++)
            {
                ...
                {
                    var x = Pre[i - 1];
                    var y = Pre[A.Count - 1] - Pre[i];
                    if (x == y)
                        Result.Add(i);
                }
            }

            int min = Result.Count > 0 ? Result.Min() : -1;
            Console.WriteLine("Min Index:- "+ min);

            Console.WriteLine("PreFix sum:-");
            Pre.ForEach(Console.WriteLine);
            Console.ReadLine();
        }
    }
}
